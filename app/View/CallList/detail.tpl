...
{html func=script url='view/calllist/call_list_detail'}
{html func=script url='pager'}

{literal}
<script>
	var list_name_old = "{/literal}{$list['T10CallList']['list_name']}{literal}";
	var enable_edit_call_list = "{/literal}{$enable_edit_call_list}{literal}";
	{/literal}
		{foreach from=$t12_list_items item=t12_list_item}
			{if $t12_list_item['T12ListItem']['item_code'] eq 'tel_no'}
				var indexTelNo = "{$t12_list_item['T12ListItem']['column']}";
			{/if}
		{/foreach}
	{literal}
</script>
...
<!-- content starts -->
	...
	<div class="row">
		<form class="form-horizontal" role="form"  action="" id="form_update_list_info">
		  <div class="form-group">
			<input type="hidden" value="{$list['T10CallList']['id']}" name="unlockEditCallList" id="call_list_id" class="input_box_new"/>
			<label class="col-sm-2 control-label">リスト名</label>
			<div class="col-sm-4">
			  <input type="text" class="form-control" value="{$list['T10CallList']['list_name']}" name="data[T10CallList][ListName]" id="call_list_name" data-rule-maxlength="100" data-msg-required="リスト名は必須項目です。" placeholder="リスト名" {if !$enable_edit_call_list}readonly{/if}/>
			</div>
		  ...
		  <div class="form-group">
			...
			<div class="col-sm-4 ptop7">
			  <input type="checkbox" name="data[T10CallList][ListTestFlag]" id="list_test_flag" {if $list['T10CallList']['list_test_flag'] eq '1'}checked{/if} {if !$enable_edit_call_list}disabled{/if}>
	...
	<div class="row">
		<div class="form-group col-md-12">
			...
			{if $enable_delete}
				<!-- 20160405 Edit by Giang - #6854 - disable double-click -->
				<a href="javascript:void(0);" title="選択項目を削除" data-toggle="tooltip" class="btn btn-default executebtn" id="btnDelTel" call_list_id="{$list['T10CallList']['id']}" tel_total="{$list['T10CallList']['tel_total']}">選択項目を削除</a>
			{/if}
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<p class="form-control-static">{$list['T10CallList']['list_name']}</p>
							<input type="text" id="id" name="id" class="hide">
						</div>
					</div>
					{foreach from=$t12_list_items item=t12_list_item}
						<div class="form-group">
							<label class="col-sm-3 control-label">{$headers[$t12_list_item['T12ListItem']['column']]}</label>
							<div class="form-group col-sm-8">
								{if $t12_list_item['T12ListItem']['item_code'] eq 'birthday'}
								<div class="row">
								...
								<input type="text" id="{$t12_list_item['T12ListItem']['column']}" name="birthday_date" class="birthday_date hide" readonly/>
								{elseif $t12_list_item['T12ListItem']['item_code'] eq 'tel_no'}
									<input type="text" id="{$t12_list_item['T12ListItem']['column']}" name="tel_number" class="form-control input-sm"/>
								{elseif $t12_list_item['T12ListItem']['item_code'] eq 'money'}
									<input type="text" id="{$t12_list_item['T12ListItem']['column']}" name="fee" class="form-control input-sm"/>
								{else}
									<input type="text" id="{$t12_list_item['T12ListItem']['column']}" name="{$t12_list_item['T12ListItem']['column']}" class="form-control input-sm"/>
								{/if}
							</div>
						</div>
					{/foreach}
