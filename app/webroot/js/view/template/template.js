...
$(document).ready(function() {
	...
	$(document).on('click', '.btnEdit', function (e) {
		...
		setSectionAudioInfo(ques_no, "sms-error-audio", "ques_sms_audio_type", "ques_audio_id");
		setSectionAudioInfo(ques_no, "sms-input-error-audio", "ques_sms_input_audio_type", "ques_sms_input_audio_id");
    	//#終了ボタンチェックボックス
...
/*					if (!glb_arr_ques[question][prefix + '99']) {
				...
				if(glb_arr_ques[question].question_type == QUESTION_VOICE || glb_arr_ques[question].question_type == QUESTION_TEL || glb_arr_ques[question].question_type == QUESTION_RECORD || glb_arr_ques[question].question_type == QUESTION_SMS || glb_arr_ques[question].question_type == QUESTION_SMS_INPUT){
					if (!glb_arr_ques[question]['jump_question']) {
				...
				if(glb_arr_ques[question].question_type == QUESTION_SMS){
					...
					if (hasLengthOverForSms(glb_arr_ques[question]['smsBodyContent'],glb_arr_ques[question]['sms_use_short_url'], question)) {
						message += "質問" + question + "のSMSセクションを確認して下さい。" + OUTBOUND_QUESTION_SMS_BODY_REACH_LIMIT + "<br>";
					}
				}
				// テンプレート詳細画面で更新をクリックしたときのバリデート
				if(glb_arr_ques[question].question_type == QUESTION_SMS_INPUT){
				...
				if(glb_arr_ques[question].question_type == QUESTION_BASIC
						...
						|| glb_arr_ques[question].question_type == QUESTION_SMS_INPUT
				){
				...
				if ((glb_arr_ques[question].question_type == QUESTION_VOICE
				...
				}else if(glb_arr_ques[question].question_type == QUESTION_SMS){
					target_keys = [ 'ques_sms_audio_type' ];
				...
				}else if(glb_arr_ques[question].question_type == QUESTION_SMS_INPUT){
					//音声のチェック
					...
					//送信不可の音声のチェック
					sms_target_keys = [ 'ques_sms_input_audio_type' ];
	...
	// URL短縮（電話番号を替えた時、短縮URLチェックボックスの状態を切替、番号指定SMS送信本文を再計算する。）
	$(document).on('click', '#slSMSInputPhoneNumber', function (e) {
		setSMSInputState();
	});
	// URL短縮（URL短縮ボタンをおした時、番号指定SMS送信本文を再計算する）
