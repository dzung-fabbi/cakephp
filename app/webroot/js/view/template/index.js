$(document).ready(function() {
	{
		...
		$("#tblTemplate").tablesorter({
			...
			type: "POST",
            async: false,
			ajaxUrl: appRoot + "Template/arr_template/{page}/20/{sortList:column}?{filterList:filter}",
			ajaxObject: {
				cache: false,
				dataType: 'json',
			},
	...
	$("#btnCreate").click(function() {
		window.location.href = appRoot+"Template/template/"
	});
	...
	$(document).on('click', '.btnEdit', function () {
		...
		$.ajax({
			type: "POST",
	        url:appRoot+"Template/check_exist_template/",
	        async: false,
	        data: {
	        ...
	        success:function(data){
	         	if(data == "err_not_exist"){
	        		...
	        		var url=appRoot+"Template/template";
	        		display_load();
	...
	$(document).on('click', '#btnDelete', function () {
		...
		$.ajax({
			type: "POST",
			url:appRoot+"Template/check_delete_template/",
	        data: {
	        ...
	        success:function(data){
				...
				if(status == "err_not_exist"){
					...
					$('#template-error-message').show();
				} else if (status == "err_exist_schedule"){
	           		//alert("選択したテンプレートは予定されているスケジュールに使用される為、削除できません。")
					$('#template-error-message').find('p').text("選択したテンプレートは予定されているスケジュールに使用される為、削除できません。");
					$('#template-error-message').show();
	           	}else{
	           		if(confirm("テンプレートを削除します。よろしいですか？")){
		    			display_load();

		    			var url=appRoot+"Template/delete";
		    			$("#formTemplate").attr('action', url);
		    			$("#formTemplate").attr('method', 'post');
		    			$("#formTemplate").attr('enctype', 'multipart/form-data');
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data == "err_not_exist"){
					...
					display_load();
					var url=appRoot+"Template/buffer_template_data";
					$.ajax({
						url: url,
						type: "post",
						data: {
						...
						success: function(result){
							if(result == "success"){
								window.location.href = appRoot + "Template/download_file/" + template_id;
							} else if (result == 'err_exist_question_sms') {
								alert(OUTBOUND_EXPORT_EXIST_QUESTION_SMS);
								location.reload();
								return;
							} else {
								window.location.href = appRoot + "Template/index";
