$(document).ready(function() {
	$(document).on('click', '#btnSubmitQues', function (e) {
		...
		var ques_auth_match_flag = false;
		var ques_no_edit = $("#btnSubmitQues").attr("ques_no");
		var sms_question_count;
		var sms_input_question_count;
		for (var ques_no_tmp in glb_arr_ques) {
			if (glb_arr_ques[ques_no_tmp]["question_type"] == QUESTION_AUTH_CHAR && glb_arr_ques[ques_no_tmp]["auth_match_flag"] == 1 && ques_no_edit != ques_no_tmp){
				ques_auth_match_flag = true;
			}
		}
		//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH - End
		var validAddQues = validate_form(ques_type, ques_auth_match_flag); //20160222 Add by Giang : #6495 - Bug 160 - focus to div error when add question
		...
		if(validAddQues){ //20160222 Edit by Giang : #6495 - Bug 160 - focus to div error when add question
			...
			if(ques_type == QUESTION_INBOUND_SMS){
				...
				for (var arr_ques_no in glb_arr_ques) {
					if(glb_arr_ques[arr_ques_no]["question_type"] == QUESTION_INBOUND_SMS){
						sms_question_count++;
					}else if(glb_arr_ques[arr_ques_no]["question_type"] == QUESTION_INBOUND_SMS_INPUT){
						sms_input_question_count++;
					}
				}
				
				//通知番号SMS、番号指定SMSが1つ以上設定されている場合、メッセージを表示する。(上限値を超える場合は無視する)
				if((sms_question_count >= 1 || sms_input_question_count >= 1) && sms_question_count <= QUESTION_INBOUND_SMS_LIMIT_COUNT){
						alert("以下の項目は他の通知番号SMSセクション、番号指定SMSセクションにも反映されます。\n"+
										"・通知番号\n"+
										"・短縮URL\n"+
										"・送信不可音声\n"
									);
			...
			if(ques_type == QUESTION_INBOUND_SMS_INPUT){
				...
				//通知番号SMS、番号指定SMSが1つ以上設定されている場合、メッセージを表示する。(上限値を超える場合は無視する)
				if((sms_question_count >= 1 || sms_input_question_count >= 1) && sms_input_question_count <= QUESTION_INBOUND_SMS_INPUT_LIMIT_COUNT){
						alert("以下の項目は他の通知番号SMSセクション、番号指定SMSセクションにも反映されます。\n"+
										"・通知番号\n"+
										"・短縮URL\n"+
										"・送信不可音声\n"
	...
	$(document).on('change', '#slQuesType', function (e) {
		...
		var count_question_property_search = 0;
		var sms_phone_number;
		var sms_short_url;
		var sms_audio_content;
		var sms_ques_no;
		
		//通知番号SMS、番号指定SMSのカウント
		//既存SMSセクションの共通項目を取得
		var sms_question_count = 0;
		var sms_input_question_count = 0;

		if(ques_type == QUESTION_INBOUND_SMS || ques_type == QUESTION_INBOUND_SMS_INPUT){
			for (var ques_no in glb_arr_ques) {
				if(glb_arr_ques[ques_no]["question_type"] == QUESTION_INBOUND_SMS){
					sms_question_count++;
					sms_phone_number = glb_arr_ques[ques_no]["smsPhoneNumber"];
					sms_short_url = glb_arr_ques[ques_no]["sms_use_short_url"];
					sms_audio_content = glb_arr_ques[ques_no]["ques_inbound_sms_audio_content"];
					sms_ques_no = ques_no;
				}else if(glb_arr_ques[ques_no]["question_type"] == QUESTION_INBOUND_SMS_INPUT){
					sms_input_question_count++;
					sms_phone_number = glb_arr_ques[ques_no]["smsInputPhoneNumber"];
					sms_short_url = glb_arr_ques[ques_no]["sms_input_use_short_url"];
					sms_audio_content = glb_arr_ques[ques_no]["ques_inbound_sms_input_audio_content"];
					sms_ques_no = ques_no;
		...
		for (var ques_no in glb_arr_ques) {
			...
			if(glb_arr_ques[ques_no]["question_type"] == QUESTION_INBOUND_COLLATION &&
					ques_type == QUESTION_INBOUND_COLLATION &&
					ques_no_edit != ques_no){
				alert("着信番号照合は既に存在します為登録できません。");
				showDialogQues(QUESTION_VOICE);
				return;
			}
			// ques_no_edit != ques_no　は今回の修正ではない質問（セクション）
			// セクションの編集で、保存ボタンをクリックせず、
			// 物件入力(賃料、平米)→違うセクション→物件入力(賃料、平米)と連続してプルダウンを切り替えたとき、
			// アラートとならないように、カウント対象から除外する。
			if(glb_arr_ques[ques_no]["question_type"] == QUESTION_PROPERTY_SEARCH &&
					ques_no_edit != ques_no){
				count_question_property_search += 1;
			}

			if(ques_type == QUESTION_INBOUND_SMS &&
					sms_question_count >= QUESTION_INBOUND_SMS_LIMIT_COUNT
				){
					alert("通知番号SMS送信は" + QUESTION_INBOUND_SMS_LIMIT_COUNT + "セクションまで登録可能です。");
					showDialogQues(QUESTION_VOICE);
					return;
			}

			if(ques_type == QUESTION_INBOUND_SMS_INPUT &&
					sms_input_question_count >= QUESTION_INBOUND_SMS_INPUT_LIMIT_COUNT
				){
				alert("番号指定SMS送信は" + QUESTION_INBOUND_SMS_INPUT_LIMIT_COUNT + "セクションまで登録可能です。");
				showDialogQues(QUESTION_VOICE);
				return;
			}
			
			//既にSMSセクションが設定されている場合、
			//2つ目以降のSMSセクションには通知番号、短縮URL、送信不可音声をデフォルト値とする。
			//※通知番号、短縮URL、送信不可音声はSMSセクションと番号指定SMSセクションは共通項目
			if(ques_type == QUESTION_INBOUND_SMS && (sms_question_count >= 1 || sms_input_question_count >= 1)){
				...
				//送信不可音声のセット
				setSectionAudioInfo(sms_ques_no, "sms-error-audio", "ques_inbound_sms_audio_type", "ques_sms_inbound_audio_id");
				$('#txtAudioSmsErrorContent').val(sms_audio_content);
				
			}

			//既に番号指定SMSセクションが設定されている場合、
			//2つ目以降のSMSセクションには通知番号、短縮URL、送信不可音声をデフォルト値とする。
			//※通知番号、短縮URL、送信不可音声はSMSセクションと番号指定SMSセクションは共通項目
			if(ques_type == QUESTION_INBOUND_SMS_INPUT && (sms_question_count >= 1 || sms_input_question_count >= 1)){
				...
				//送信不可音声のセット
				setSectionAudioInfo(sms_ques_no, "sms-input-error-audio", "ques_inbound_sms_input_audio_type", "ques_sms_input_inbound_audio_id");
		...
		// 新たに物件入力(賃料、平米)の追加を行う場合
		if(ques_type == QUESTION_PROPERTY_SEARCH){
			limit_count = 6;
			count_question_property_search += 1;
			if(count_question_property_search >= limit_count){
					alert("物件入力(賃料、平米)は" + limit_count + "問以上登録できません。");
					showDialogQues(QUESTION_VOICE);
					return;
				}
		}
...
// セグメントの作成・編集画面から、テンプレート詳細画面への転送
// app/View/InboundTemplate/template.tpl　で作成したセグメントの部分と合わせること。
function appendQues(ques_no) {
	var timeout_reflush_flag = false;
	//var ques_auth_match_flag = false;//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH
	//質問NO
	if(ques_no){
		ques_no = ques_no;
		...
		//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH - Begin
		/*if ($('#slQuesType').val() == QUESTION_AUTH && $("#dialog").find("#auth_match_flag").is(':checked')) {
			row.appendTo(".ques_auth_match_flag");
		} else if (glb_arr_ques[ques_no].question_type == QUESTION_AUTH && glb_arr_ques[ques_no].auth_match_flag == 1) {
			row.prependTo(".template");
		}*/
		//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH - End
	}else{
		var counter = 0;
		for (var i in glb_arr_ques) {
			...
			if (glb_arr_ques[i].question_type == QUESTION_TIMEOUT){
		...
		ques_no = counter + 1;
		if($('#slQuesType').val() == QUESTION_TIMEOUT) {
			row.appendTo(".timeout");
			//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH - Begin
		/*} else if ($('#slQuesType').val() == QUESTION_AUTH && $("#dialog").find("#auth_match_flag").is(':checked')) {
			row.appendTo(".ques_auth_match_flag");*/
			//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH - End
		} else if ($('#slQuesType').val() == QUESTION_INBOUND_COLLATION){
			//着信照合の場合は先頭に追加する。	
			row.prependTo(".inbound_collation");
		} else{
			row.appendTo(".template");
		}
	}
	//質問数チェック
	if(ques_no > IN_MAX_SECTION_COUNT){
		alert("テンプレート上の最大セクション数は" + IN_MAX_SECTION_COUNT + "の為、これ以上のセクション追加ができません。");
	}else{
		...
		$($("#form_add_call_list").serializeArray()).each(function(i, v) {
			...
			//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH - Begin
			/*if ($('#slQuesType').val() == QUESTION_AUTH && v.name == 'auth_match_flag' && v.value == 1) {
				ques_auth_match_flag = true;
			}*/
			//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH - End
		});

		//20160407 Add by Thai : #6898 - Bug update QUESTION_AUTH - Begin
		...
		//20160407 Add by Thai : #6898 - Bug update QUESTION_AUTH - End

		// console.log(glb_arr_ques);
		// 今回作成・変更したセグメントを、テンプレート詳細画面に送る(描画する)
		...
		}else if(question_type == QUESTION_PROPERTY){
			htmlQuesProperty(row, glb_arr_ques[ques_no], ques_no);
		}else if(question_type == QUESTION_FAX){
			htmlQuesInputFax(row, glb_arr_ques[ques_no], ques_no);
		}else if(question_type == QUESTION_PROPERTY_SEARCH){
			htmlQuesPropertySearch(row, glb_arr_ques[ques_no], ques_no);
		}else if(question_type == QUESTION_INBOUND_SMS){
			//他のSMSセクションの共通項目も変更する。
			for (var tmp_arr in glb_arr_ques) {
				if (glb_arr_ques[tmp_arr].question_type == QUESTION_INBOUND_SMS || glb_arr_ques[tmp_arr].question_type == QUESTION_INBOUND_SMS_INPUT){
					//通知番号SMSプロパティを更新
					glb_arr_ques[tmp_arr].smsPhoneNumber = glb_arr_ques[ques_no].smsPhoneNumber;
					glb_arr_ques[tmp_arr].ques_inbound_sms_audio_type = glb_arr_ques[ques_no].ques_inbound_sms_audio_type;
					glb_arr_ques[tmp_arr].ques_sms_inbound_audio_id = glb_arr_ques[ques_no].ques_sms_inbound_audio_id;
					glb_arr_ques[tmp_arr].ques_sms_inbound_audio_name = glb_arr_ques[ques_no].ques_sms_inbound_audio_name;
					glb_arr_ques[tmp_arr].ques_inbound_sms_audio_content = glb_arr_ques[ques_no].ques_inbound_sms_audio_content;
					...
					glb_arr_ques[tmp_arr].ques_inbound_sms_input_audio_type = glb_arr_ques[ques_no].ques_inbound_sms_audio_type;
					glb_arr_ques[tmp_arr].ques_sms_input_inbound_audio_id = glb_arr_ques[ques_no].ques_sms_inbound_audio_id;
					glb_arr_ques[tmp_arr].ques_sms_input_inbound_audio_name = glb_arr_ques[ques_no].ques_sms_inbound_audio_name;
					glb_arr_ques[tmp_arr].ques_inbound_sms_input_audio_content = glb_arr_ques[ques_no].ques_inbound_sms_audio_content;
					glb_arr_ques[tmp_arr].sms_input_use_short_url = glb_arr_ques[ques_no].sms_use_short_url;
				}
			}
			htmlQuesInboundSms(row, glb_arr_ques[ques_no], ques_no);
		}else if(question_type == QUESTION_INBOUND_COLLATION){
			htmlQuesInboundCollation(row, glb_arr_ques[ques_no], ques_no);
			//着信番号照合は先頭に追加するため、追加時に並び順を更新する
			update_glb_arr_ques();
		}else if(question_type == QUESTION_INBOUND_SMS_INPUT){
			for (var tmp_arr in glb_arr_ques) {
				if (glb_arr_ques[tmp_arr].question_type == QUESTION_INBOUND_SMS || glb_arr_ques[tmp_arr].question_type == QUESTION_INBOUND_SMS_INPUT){
					//通知番号SMSプロパティを更新
					glb_arr_ques[tmp_arr].smsPhoneNumber = glb_arr_ques[ques_no].smsInputPhoneNumber;
					glb_arr_ques[tmp_arr].ques_inbound_sms_audio_type = glb_arr_ques[ques_no].ques_inbound_sms_input_audio_type;
					glb_arr_ques[tmp_arr].ques_sms_inbound_audio_id = glb_arr_ques[ques_no].ques_sms_input_inbound_audio_id;
					glb_arr_ques[tmp_arr].ques_sms_inbound_audio_name = glb_arr_ques[ques_no].ques_sms_input_inbound_audio_name;
					glb_arr_ques[tmp_arr].ques_inbound_sms_audio_content = glb_arr_ques[ques_no].ques_inbound_sms_input_audio_content;
					...
					glb_arr_ques[tmp_arr].ques_inbound_sms_input_audio_type = glb_arr_ques[ques_no].ques_inbound_sms_input_audio_type;
					glb_arr_ques[tmp_arr].ques_sms_input_inbound_audio_id = glb_arr_ques[ques_no].ques_sms_input_inbound_audio_id;
					glb_arr_ques[tmp_arr].ques_sms_input_inbound_audio_name = glb_arr_ques[ques_no].ques_sms_input_inbound_audio_name;
					glb_arr_ques[tmp_arr].ques_inbound_sms_input_audio_content = glb_arr_ques[ques_no].ques_inbound_sms_input_audio_content;
					glb_arr_ques[tmp_arr].sms_input_use_short_url = glb_arr_ques[ques_no].sms_input_use_short_url;
				}
			}
			htmlQuesInboundSmsInput(row, glb_arr_ques[ques_no], ques_no);
		}

		//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH - Begin
		/*if (ques_auth_match_flag) {
			//$(".template .row_question:eq(" + (ques_no - 1) + ")").insertBefore($(".template .row_question:eq(0)"));
			update_glb_arr_ques();
		}*/
		//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH - End
...
function htmlRecheckAudio2(recheck_flag, audio_id, audio_type, audio_name, audio_content){
	var recheck = [];
	if(recheck_flag == 0){
		recheck['tmp'] = '<p>なし</p>';
		recheck['content'] = '';
...
function htmlAnswerAuth(data){
	...
	//20160418 Edit by Thai : #6722 - jump_question of QUESTION_AUTH - Begin
	//20160420 Delete by Thai : #6722 remove item auth_match_flag of QUESTION_AUTH - Begin
/*	if (data["auth_match_flag"] == 1) {
		//20160224 Add by Thai : #6519 - Update select jump question - Begin
		tmp = tmp +
			'<tr>' +
				'<td colspan="3">リスト非存在</td>' +
				'<td class="alignCenter select_jump_ques_container" jump_question="' + data['jump_question'] + '"></td>' +
			'</tr>';
		//20160224 Add by Thai : #6519 - Update select jump question - End
	} else {*/
	//20160420 Delete by Thai : #6722 remove item auth_match_flag of QUESTION_AUTH - End
...
function htmlTableAnswerProperty(data){
	var answer_property = "";
	var table_answer_property = "";
	var bukken_cont_answer_no = data['bukken_cont_answer_no'];
	if(bukken_cont_answer_no == '51')
		bukken_cont_answer_no = '*';
	else if(bukken_cont_answer_no == '52')
		bukken_cont_answer_no = '#';
	answer_property = answer_property +
		'<tr>' +
		'<td>確認を続ける</td>' +
		'<td class="alignCenter">' + bukken_cont_answer_no + '</td>' +
		'<td class="alignCenter select_jump_ques_container" jump_question="' + data['txtAnswJumpProp0'] + '" ans_no="' + 0 + '">' + '</td>' +
		'</tr>';
	answer_property = answer_property +
		'<tr>' +
		'<td>確認を続けない</td>' +
	...
	answer_property = answer_property +
		'<tr class="fixHeight" style="height:50px;"></tr><tr>' +
		'<td colspan="2">共通タイムアウト設定</td>' +
		'<td class="alignCenter select_jump_ques_container" jump_question="' + data['txtAnswJumpProp99'] + '" ans_no="99">' + '</td>' +
		'</tr>';

	table_answer_property = '<div class="row">' +
		'<div class="col-md-8">' +
		'<p>回答</p>' +
...
function htmlAnswerAuthChar(data){
	...
	//20160427 Edit by Thai : #6722 edit QUESTION_AUTH_CHAR - Begin
	tmp = 	tmp +
		'<tr>' +
			'<td colspan="3">タイムアウト</td>' +
			'<td class="alignCenter select_jump_ques_container" jump_question="' + data['txtAnswJumpAuthChar99'] + '" ans_no="99">' + '</td>' +
		'</tr>';
	//20160427 Edit by Thai : #6722 edit QUESTION_AUTH_CHAR - End
	return tmp;
}
//20160420 Add by Thai : #6722 ADD QUESTION_AUTH_CHAR - End

function htmlAnswerInboundCollation(data){
	var arr_answ_title = ["通知番号 ＝ 認証項目", "通知番号 ≠ 認証項目", 99];
	var tmp = "";
	for(var i=1; i <= arr_answ_title.length; i++){
		if(i != arr_answ_title.length){
			tmp = 	tmp +
					'<tr>' +
						'<td class="alignCenter">' + arr_answ_title[i-1] + '</td>' +
						'<td class="alignCenter select_jump_ques_container" jump_question="' + data['txtAnswJumpInboundCollation' + i] + '" ans_no="' + i + '">' + '</td>' +
					'</tr>';
		}
	}
	return tmp;
...
function htmlHeader(data, ques_no, ques_type_txt){
	...
	if(ques_type_txt != "タイムアウト" && ques_type_txt != "着信番号照合") {
...
/*	if(data["auth_match_flag"] > 0){
		var auth_match_flag = "あり";
	}else{
		var auth_match_flag = "なし";
	}*/
	//20160420 Delete by Thai : #6722 remove item auth_match_flag of QUESTION_AUTH - End
	//20160406 Add by Thai : #6722 - Add item for QUESTION_AUTH - End
	var html = 	'<div class="box col-md-12">' +
...
function htmlQuesInboundCollation(row, data, ques_no){
	var header = htmlHeader(data, ques_no, getQuesTypeTxtByQuesType(QUESTION_INBOUND_COLLATION));
	var answer_inbound_collation = htmlAnswerInboundCollation(data);
	var html = 	'<div class="box col-md-12">' +
					'<div class="box-inner">' +
						header +
						'<div class="box-content">' +
							...
							'<div class="row">' +
								'<div class="col-md-8">' +
									'<table class="table table-bordered table-striped table-condensed">' +
										'<thead>' +
											'<tr>' +
												'<th class="alignCenter templateTable-240">結果</th>' +
												'<th class="alignCenter col-md-3">飛び先</th>' +
											'</tr>' +
										'</thead>' +
										'<tbody>' +
											answer_inbound_collation +
										'</tbody>' +
									'</table>' +
								'</div>' +
							'</div>' +
...
function htmlQuesInboundSmsInput(row, data, ques_no){
	var audio = htmlAudio(data["audio_id"], data["audio_type"], data["audio_name"], data["audio_content"]);
	var header = htmlHeader(data, ques_no, getQuesTypeTxtByQuesType(QUESTION_INBOUND_SMS_INPUT));
	...
	var error_audio = htmlAudio(data["ques_sms_input_inbound_audio_id"], data["ques_inbound_sms_input_audio_type"], data["ques_sms_input_inbound_audio_name"], data["ques_inbound_sms_input_audio_content"]);
	var html = '' +
...
function htmlQuesProperty(row, data, ques_no){
	var audio = htmlAudio(data["audio_id"], data["audio_type"], data["audio_name"], data["audio_content"]);
	var bukken_audio = htmlAudio(data["bukken_audio_id"], data["bukken_audio_type"], data["bukken_audio_name"], data["bukken_audio_content"]);
	var bukken_diagram_audio = htmlAudio(data["bukken_diagram_audio_id"], data["bukken_diagram_audio_type"], data["bukken_diagram_audio_name"], data["bukken_diagram_audio_content"]);
	var bukken_cont_audio = htmlAudio(data["bukken_cont_audio_id"], data["bukken_cont_audio_type"], data["bukken_cont_audio_name"], data["bukken_cont_audio_content"]);
	var header = htmlHeader(data, ques_no, getQuesTypeTxtByQuesType(QUESTION_PROPERTY));
	var table_answer_bukken_cont_property = htmlTableAnswerProperty(data);
	var bukken_answer_no = data['bukken_answer_no'];
	if(bukken_answer_no == '51')
		bukken_answer_no = '*';
	else if(bukken_answer_no == '52')
		bukken_answer_no = '#';	
	var bukken_diagram_answer_no = data['bukken_diagram_answer_no'];
	if(bukken_diagram_answer_no == '51')
		bukken_diagram_answer_no = '*';
	else if(bukken_diagram_answer_no == '52')
		bukken_diagram_answer_no = '#';
	var html = '<div class="box col-md-12">' +
		'<div class="box-inner">' +
		header +
		...
		'<div class="col-md-10">' + bukken_audio + '</div>' +
		'</div>' +
		'<div class="row">' +
		'<div class="col-md-8">' +
		'<p>回答</p>' +
		...
		'<tr>' +
		'<td>物件名が正しい場合</td>' +
		'<td class="alignCenter">' + bukken_answer_no + '</td>' +
		'</tr>' +
		'<tr>' +
		'<td>物件名を訂正する場合</td>' +
		'<td class="alignCenter">その他</td>' +
		...
		'<div class="col-md-10">' + bukken_diagram_audio + '</div>' +
		'</div>' +
		'<div class="row">' +
		'<div class="col-md-8">' +
		'<p>回答</p>' +
		...
		'<tr>' +
		'<td>図面を希望する場合</td>' +
		'<td class="alignCenter">' + bukken_diagram_answer_no + '</td>' +
		'</tr>' +
		'<tr>' +
		'<td>図面を希望しない場合</td>' +
		'<td class="alignCenter">その他</td>' +
		...
		'<div class="col-md-10">' + bukken_cont_audio + '</div>' +
		'</div>' +
		table_answer_bukken_cont_property +
		'</div>' +
		'</div>' +
		'</div>';
	$(row).append(html);
}
function htmlQuesInputFax(row, data, ques_no){
	var audio = htmlAudio(data["audio_id"], data["audio_type"], data["audio_name"], data["audio_content"]);
	var header = htmlHeader(data, ques_no, getQuesTypeTxtByQuesType(QUESTION_FAX));
	if(data["recheck_flag"]){
		var recheck_button_next = data["recheck_button_next"];
		if(recheck_button_next == '51')
			recheck_button_next = '*';
	...
	var recheck = htmlRecheckAudio2(recheck_flag, data["recheck_audio_id"], data["recheck_audio_type"], data["recheck_audio_name"], data["recheck_audio_content"]);
	var html = '<div class="box col-md-12">' +
		'<div class="box-inner">' +
		header +
		'<div class="box-content">' +
		...
		'<p>タイムアウト</p>' +
		'</div>' +
		'<div class="col-md-2 select_jump_ques_container" jump_question="' + data['txtAnswJumpFax99'] + '" ans_no="99">' + '"></div>' +
		'</div>' +
		'<div class="row">' +
		'<div class="col-md-2"><p>音声</p></div>' +
		'<div class="col-md-10">' + audio + '</div>' +
		...
		'<div class="row">' +
		'<div class="col-md-2"><p>桁数</p></div>' +
		'<div class="col-md-10">' + data["digit_fax"] + '</div>' +
		'</div>' +
		recheck['content'] +
		button_next +
		'</div>' +
...
// 物件入力(賃料、平米)
// セクションの追加・編集画面で保存を押したとき、テンプレート詳細画面を描画しなおす。
function htmlQuesPropertySearch(row, data, ques_no){
	// 音声部分の作成
	var audio = htmlAudio(data["ques_property_cost_audio_id"],
							data["ques_property_cost_audio_type"],
							data["ques_property_cost_audio_name"],
							data["ques_property_cost_audio_content"]);

    var square_audio = htmlAudio(data["ques_property_square_audio_id"],
    						data["ques_property_square_audio_type"],
    						data["ques_property_square_audio_name"],
    						data["ques_property_square_audio_content"]);
    var bukken_audio = htmlAudio(data["ques_property_confirm_audio_id"],
    						data["ques_property_confirm_audio_type"],
    						data["ques_property_confirm_audio_name"],
    						data["ques_property_confirm_audio_content"]);

    var bukken_cont_audio = htmlAudio(data["ques_property_continue_audio_id"],
    						data["ques_property_continue_audio_type"],
    						data["ques_property_continue_audio_name"],
    						data["ques_property_continue_audio_content"]);
    var table_answer_bukken_cont_property = htmlTableAnswerProperty(data);



    var bukken_answer_no = data['ques_property_confirm_answer_no'];
    if(bukken_answer_no == '51')
        bukken_answer_no = '*';
    else if(bukken_answer_no == '52')
        bukken_answer_no = '#';
	...
	// ヘッダー部分の作成
	var header = htmlHeader(data, ques_no, getQuesTypeTxtByQuesType(QUESTION_PROPERTY_SEARCH));
	var html = '<div class="box col-md-12">' +
		'<div class="box-inner">' +
		header +
		'<div class="box-content">' +
            ...
            '<div class="row">' +
                ...
                '<div class="col-md-10">' + square_audio + '</div>' +
            '</div>' +
            '<div class="row">' +
            '<div class="col-md-2"><p>桁数</p></div>' +
...
function renderSelectJumpQues() {
	...
	$('.select_jump_ques_container').each(function (e) {
		...
		$('.box-header').each(function (index) {
			...
			if (question_type != QUESTION_TIMEOUT && question_type != QUESTION_INBOUND_COLLATION) {
...
function getQuesTypeByQuesTypeTxt(ques_type_txt) {
	...
	var arr_ques_type = {
		...
		'物件番号入力' : QUESTION_PROPERTY,
		'FAX番号入力' : QUESTION_FAX,
        '物件入力(賃料、平米)' : QUESTION_PROPERTY_SEARCH,
        '通知番号SMS送信' : QUESTION_INBOUND_SMS,
		'着信番号照合' : QUESTION_INBOUND_COLLATION,
		'番号指定SMS送信' : QUESTION_INBOUND_SMS_INPUT
	};
	return arr_ques_type[ques_type_txt];
}

//20160420 Add by Thai : #6722 ADD QUESTION_AUTH_CHAR - Begin
// templateのヘッダー部分を作成する関数
function getQuesTypeTxtByQuesType(ques_type) {
	...
	arr_ques_type_txt[QUESTION_PROPERTY] = '物件番号入力';
	arr_ques_type_txt[QUESTION_FAX] = 'FAX番号入力';
	arr_ques_type_txt[QUESTION_PROPERTY_SEARCH] = '物件入力(賃料、平米)';
	arr_ques_type_txt[QUESTION_INBOUND_SMS] = '通知番号SMS送信';
	arr_ques_type_txt[QUESTION_INBOUND_COLLATION] = '着信番号照合';
	arr_ques_type_txt[QUESTION_INBOUND_SMS_INPUT] = '番号指定SMS送信';
...
function updateDataWhenSelectJumpQues(element) {
	...
	if (typeof ans_no != 'undefined') {
		if (ques_type == QUESTION_BASIC) {
			...
			glb_arr_ques[ques_no]['txtAnswJumpFax' + ans_no] = element.val();
		} else if (ques_type == QUESTION_PROPERTY) {
			glb_arr_ques[ques_no]['txtAnswJumpProp' + ans_no] = element.val();
		...
		} else if (ques_type == QUESTION_INBOUND_COLLATION) {
			glb_arr_ques[ques_no]['txtAnswJumpInboundCollation' + ans_no] = element.val();
...
// 
function htmlQuesInboundSms(row, data, ques_no) {
	var header = htmlHeader(data, ques_no, getQuesTypeTxtByQuesType(QUESTION_INBOUND_SMS));
	var sms_use_short_url_flag = data["sms_use_short_url"];
	var sms_use_short_url_val = "";
	...
	var audio = htmlAudio(data["ques_sms_inbound_audio_id"], data["ques_inbound_sms_audio_type"], data["ques_sms_inbound_audio_name"], data["ques_inbound_sms_audio_content"]);
	var html = '' +
		'<div class="box col-md-12">' +
			'<div class="box-inner">' +
				header +
...
function setSectionAudioInfo(ques_no, terget_html_id, audio_type, audio_id) {
	...
	if(audio_type_value == 1 || audio_type_value == 2){
		...
		if(terget_html_id == "#sms-error-audio"){
			$(terget_html_id).find(".hdAudioName").val(glb_arr_ques[ques_no]["ques_sms_inbound_audio_name"]);
		}else{
			$(terget_html_id).find(".hdAudioName").val(glb_arr_ques[ques_no]["ques_sms_input_inbound_audio_name"]);
		}
	}
}
