...
def downloadOutbound(mysql_cli, start_time, end_time, path_base, path_base_backup)
	...
	file_zip_name = "アウトバウンド_" + year_of_prev_2_month.to_s + moth_of_prev_2_month.to_s + ".zip"
	...
	Zip::File.open(path, Zip::File::CREATE) do |zipfile|
		...
		for company_id in company_ids
			...
			file_name = company_id[1].to_s + "_アウトバウンド_" + year_of_prev_2_month.to_s + moth_of_prev_2_month.to_s + ".csv"
...
def downloadInbound(mysql_cli, start_time, end_time, path_base, path_base_backup)
	...
	non_date = '0000-00-00 00:00:00'
	file_zip_name = "インバウンド_" + year_of_prev_2_month.to_s + moth_of_prev_2_month.to_s + ".zip"
	path = path_base + file_zip_name
	file_list = []
	Zip::File.open(path, Zip::File::CREATE) do |zipfile|
		Zip.unicode_names = true
		for company_id in company_ids
			...
			file_name = company_id[1].to_s + "_インバウンド_" + year_of_prev_2_month.to_s + moth_of_prev_2_month.to_s + ".csv"
			file_path = path_base + file_name
			FileUtils.cd(path_base) do
				CSV.open(file_name, "w", encoding: "SJIS") do |csv|
					csv << header_lists
...
def downloadSms(mysql_cli, start_time, end_time, path_base, path_base_backup)
	...
	file_zip_name = "SMS_" + year_of_prev_2_month.to_s + moth_of_prev_2_month.to_s + ".zip"
	path = path_base + file_zip_name
	file_list = []
	Zip::File.open(path, Zip::File::CREATE) do |zipfile|
		Zip.unicode_names = true
		for company_id in company_ids
			...
			file_name = company_id[1].to_s + "_SMS_" + year_of_prev_2_month.to_s + moth_of_prev_2_month.to_s + ".csv"
			file_path = path_base + file_name
			FileUtils.cd(path_base) do
				CSV.open(file_name, "w", encoding: "SJIS") do |csv|
					csv << header_lists
