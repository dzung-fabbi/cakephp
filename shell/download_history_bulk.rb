...
def downloadOutbound(mysql_cli, start_time, end_time, path_base, path_base_backup)
	...
	#Delele file
	prev_6_month = DateTime.now.prev_month(8)
	...
	Zip::File.open(path, Zip::File::CREATE) do |zipfile|
		...
		for company_id in company_ids
			...
			for schedule in schedules
				...
				logs = getAllByScheduleId(mysql_cli, schedule_id, tel_colum, start_time, end_time)
				for log in logs
					data = Array.new()
	...
	Zip::File.open(path, Zip::File::CREATE) do |zipfile|
		...
		for company_id in company_ids
			...
			for inbound in inbounds
				...
				download_log_tmp_question_all_type = getT64QuesNumByScheduleId(mysql_cli, inbound_id, true)
...
def getAllByScheduleId(mysql_cli, schedule_id, tel_colum, start_time, end_time)
	query = <<EOS
	select
		t80.*,
		t51.*
...
def getAnswerPos(mysql_cli, inbound_id)
	...
	for question_number in question_numbers
		...
		if count_column > 0
			arr_answer_pos[question_no.to_i] = current_pos
			current_pos += count_column
		elsif question_type == '5'
			arr_answer_pos[question_no.to_i] = 'trans_call_time'
		else
			arr_answer_pos[question_no.to_i] = nil
		end
	end
...
def getT64QuesNumByScheduleId(mysql_cli, inbound_id, question_type_put_is = false)
	query = <<-EOS
		select
			t64.question_title,
			t64.question_no,
...
#######################################################
begin
	program_name = "[結果ログ一括DLの全社分ファイル作成]"
	#DB接続情報
