...
def getSendingInfo(mysql_cli)
	...
	query = <<EOS
		select  t83.log_id,
				...
				m08.api_id
		from t83_outgoing_sms_statuses t83
		join m08_sms_api_infos m08
		on t83.company_id = m08.company_id
		and t83.display_number = m08.display_number
		and ((m08.role_code = '20' and m08.api_id != '2') or m08.api_id = '2')
		where t83.sms_status = 'sending'
		and t83.del_flag = 'N'
		and m08.del_flag = 'N'
		and m08.api_id = '2';
EOS
...
def getSmsStatusFromApi_V2(sms_api_config, entry_id)
	...
	writeLog("getSmsStatusFromApi_V2::#{entry_id}::#{result}")
	return result
end
...
# @author 	: kuniyoshi
#=============================================================================
def updateSmsStatus_V2(mysql_cli, send_result, log_id, schedule_id, template_id, sms_question_no, sms_entry_id)
	warning_msg = '不明'
	status = 'fail'
	sms_short_url_key = ''
...
# @author 	: Hungnv
#=============================================================================
def updateSmsStatus(mysql_cli, send_result, log_id, schedule_id, template_id, sms_question_no, sms_entry_id)
	warning_msg = '不明'
	status = 'fail'
...
# @author 	: Hungnv
#=============================================================================
def insertGetSmsStatusHistories(mysql_cli, result)
	if result.nil?
		return false
	else
		rs = result["Result"]
		...
		query = <<EOS
			insert into t84_outgoing_getsmsstatus_histories(entry_id, ResStatus, ResCount,create_date, req_stat,
...
# @author 	: kuniyoshi
#=============================================================================
def insertGetSmsStatusHistories_V2(mysql_cli, result, sending_info_row)
	if result.nil?
		return false
	else
		...
		if carrier_id == "5"
			...
			message = escape_str(getCareerMsg_V2(result))
		end
		...
		query = <<EOS
			insert into t84_outgoing_getsmsstatus_histories(entry_id, ResStatus, create_date, 
...
begin
	...
	smsSendingData = getSendingInfo(mysql_cli)
	if smsSendingData.length > 0
		smsSendingData.each do | row |
			...
			if(sms_api_config["api_id"] == $SMS_API_V2_VALUE)
				send_result = getSmsStatusFromApi_V2(sms_api_config, sms_entry_id)
				writeLog(program_name + schedule_no + " SMS-APIv2ステータス取得 ：　OK")
				insertGetSmsStatusHistories_V2(mysql_cli, send_result, row)
				writeLog(program_name + schedule_no + " SMS-APIv2ステータス取得ログをインサート ：　OK")
				updateSmsStatus_V2(mysql_cli, send_result, log_id, schedule_id, template_id, sms_question_no, sms_entry_id)
				writeLog(program_name + schedule_no + " SMS-APIv2ステータス更新 ：　OK")
			# 2019-12-25 V1ステータス更新停止
			#else
				#send_result = getSmsStatusFromApi(sms_api_config, sms_entry_id)
				#writeLog(program_name + schedule_no + " SMSステータス取得 ：　OK")
				#insertGetSmsStatusHistories(mysql_cli, send_result)
				#writeLog(program_name + schedule_no + " SMSステータス取得ログをインサート ：　OK")
				#updateSmsStatus(mysql_cli, send_result, log_id, schedule_id, template_id, sms_question_no, sms_entry_id)
				#writeLog(program_name + schedule_no + " SMSステータス更新 ：　OK")
			end
		end
	end
	mysql_cli.close()
rescue Exception => e
	writeLog(program_name + "エラー：SMSステータス取得バッチ実行：失敗 - " + e.message)
