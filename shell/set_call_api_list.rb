...
limit = 5
begin
	program_name = "[API発信リスト登録]"
	server_id = ARGV[0]
	schedule_no = ARGV[1]
	schedule_id = ARGV[2]
	template_id = ARGV[3]
	list_id = ARGV[4]
	arr_tel = ARGV[5].split(',')
	#DB接続情報
...
EOS
	...
	if retried < limit
		...
		puts "err_set_call_api_list"
		writeLog(program_name + "err_set_call_api_list : " + e.message)
		writeLog(program_name + "API発信リスト登録のエラーを発生")
		writeLog(e.backtrace.join("\n"))
