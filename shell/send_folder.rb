...
begin
	...
	remotePathBackup = config[:remote_path] + schedule_no + "_" + "updateschedule_backup"
	query = <<EOS
		select 
			server_ip,
			username,
...
EOS
	mysql_cli.query(query).each do | row |
		...
		Net::SSH.start( server_ip, server_user, options) do |ssh|
			if action == "update"
				ssh.exec!("mv " + remotePathScheduleId + " " + remotePathBackup)
				writeLog(program_name + schedule_no + "フォルダバックアップ ： OK")
			end
			begin
				writeLog(program_name + schedule_no + "フォルダ転送 ： START")
				ssh.sftp.connect do |sftp|
					sftp.mkdir! remotePathScheduleId
					sftp.upload!(localPathScheduleId, remotePathScheduleId)
				end
	...
	if retried < limit
		...
		puts "err_send_folder"
		writeLog(program_name + "err_send_folder : " + e.message)
		writeLog(program_name + "フォルダを転送のエラーを発生")
